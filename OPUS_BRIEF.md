...
**Next → (highest value first)** — remaining §9 layers (L5 sightlines/privacy — ray-casts from door/windows, TV viewing angle; L7 outlet layer — marker type exists; L8 layout-diff ghost — overlay a 2nd saved plan); §4 delights (D2 sun time-lapse now trivial on the sunHours beam machinery, D4 before/after slider, D3 robot-vacuum reachability — flood-fill reusing walkGrid); the **real-device mobile pass** (P1 — §8 pre-verified the chrome; only gestures/pinch/export remain). Doorway v3 candidates if ever wanted: per-corner blame, polygon rooms decomposed, tilt/on-end 3D escapes.
