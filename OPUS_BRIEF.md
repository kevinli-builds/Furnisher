...
**Next → (highest value first)** — remaining §9 layers (L4 budget/move-day — furniture has `price`, zero geometry, fastest; L5 sightlines/privacy — ray-casts; L7 outlet layer; L8 layout-diff ghost); the **real-device mobile pass** (P1 — §8 pre-verified the chrome; only gestures/pinch/export remain). Doorway v3 candidates if ever wanted: per-corner blame in the issue copy, polygon rooms decomposed instead of bbox'd, tilt/on-end 3D escapes.
